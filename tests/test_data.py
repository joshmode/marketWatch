...
def test_cache_path_traversal():
    from app.data import cache_file_path

    # Path traversal characters should be sanitized away.
    # Check that they result in safe paths within cache directory.
    path1 = cache_file_path("../../etc/passwd", "1d")
    assert path1.name == "______etc_passwd_1d.pkl"
    assert path1.parent.name == "cache"

    path2 = cache_file_path("GSPC", "../1d")
    assert path2.name == "GSPC_.._1d.pkl"
    assert path2.parent.name == "cache"

    # Should not raise for valid inputs
    valid_path = cache_file_path("GSPC", "1d")
    assert valid_path.name == "GSPC_1d.pkl"
    assert valid_path.parent.name == "cache"
